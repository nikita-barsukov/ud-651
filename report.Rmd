...
```{r, results='asis', echo=FALSE}
knitr::kable(format(brkdn, big.mark=','), 
             align='r', 
             caption='Reported crime in Chicago and Los Angeles, 2013')
```
...
Some patterns are the same across the cities and crime types. For example, lowest crime rate is during early morning hours between 4 and 6 AM. Also it is surprizing that crime reports tend to be reported more at odd hours, as we see from jagged lines in all the facets.
...
This is the most interesting plot so far. Both personal and property crimes in Los Angeles are concentrated in a single area around Skid Row and Downton Los Angeles. In Chicago though different types of crime are concentrated in completely different areas. Property crimes are clustered around Chicago city center: Near North Side, Chicago Loop and River North. Personal crimes are concentrated heavily around western areas of Chicago: North and South Lawndale, Near West Side.
...
````{r echo=FALSE}
dd = demo_data[c("crime_all","crime_property","crime_personal","density","median_income","unemployed","education")]
mtrx = cor(dd)
mtrx = mtrx[4:7,1:3]
rownames(mtrx) = c('Population density', 
                   'Median Income',
                   'Share of unemployed',
                   'Share of at least a year in college')
colnames(mtrx) = c('Total crimes', 'Property crimes', 'Personal crimes')
knitr::kable(format(mtrx,big.mark=',', digits=2 ), 
             align='r', 
             caption='Correlations between crime reports and demography, entire dataset')
```
